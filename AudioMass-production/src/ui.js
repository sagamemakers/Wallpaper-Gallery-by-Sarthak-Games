(function ( w, d, PKAE ) {
	...
	function _makeUIMainView ( UI, app ) {
		...
		// Move volume meter to upper right, outside footer
		volume_parent.classList.add('flipaud-volume-indicator');
		// Remove dB markers for compactness
		markers.style.display = 'none';
		d.body.appendChild(volume_parent);
	...
	function _makeUIToolbar (UI) {
		...
		UI.listenFor ('DidAudioProcess', function( val ) {
			...
			if (!loudness)
			{
				...
				UI.footer.volumeGaugeInner.style.height = '0%';
				UI.footer.volumeGaugeInner2.style.height = '0%';
			}
			else if (loudness[0] > 0) {
				UI.footer.volumeGaugePeaker.className = 'pk_peaker pk_act';
				
				UI.footer.volumeGaugeInner.style.height = '100%';
				volume1 = 100;
				...
				if (loudness[1] > 0) {
					...
					UI.footer.volumeGaugeInner2.style.height = '100%';
					volume2 = 100;
			...
			else
			{
				...
				tmp = (100 + loudness[1]);
				if (tmp < -100) volume2 = 0;
				else {
					volume2 = volume2 + (tmp - volume2)/4;
					if (isNaN (volume2)) volume2 = 0;
				}

				UI.footer.volumeGaugeInner.style.height = volume1 + '%';
				UI.footer.volumeGaugeInner2.style.height = volume2 + '%';
			}
		});
